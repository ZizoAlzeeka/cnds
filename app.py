...
def port(url):
    try:
        domain = extract_domain(url)       
        api_key = "471d5b418e6b232ef4e959bbd270e3bd9ad4b62c"
...
def url_of_anchor(response, url):
    if response == "":
        ...
        # استخدام tldextract لاستخراج أجزاء URL
        extract_result = tldextract.extract(url)
        websiteDomain = extract_result.domain
        ...
        for anchor in anchors:
            try:
                extract_result = tldextract.extract(anchor['href'])
                anchorDomain = extract_result.domain
...
def WebsiteTraffic(url):
    ...
    api_key = '00c1ee7d9967427daf05b95ad1bf127e'  # استبدل بـ API Key الخاص بك من Similarweb
    try:
...
def googleIndex(url):
    ...
    api_key='237db76b4177734cb461a335ae6abf3a'
    query = f'site:{domain}'
    payload = {
        'api_key': api_key,
